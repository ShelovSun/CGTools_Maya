...
DEFAULT_FILE_TYPE = "mayaAscii"  # "mayaBinary"
...
def saveAnim(
    ...
    if sequencePath:
        if os.path.lexists(path+'/sequence'):
            os.remove(path+'/sequence')
        shutil.move(sequencePath, path)

    # Save the animation to the temp location
    anim = utils.Animation.fromObjects(objects)# 来自Animation的父类Pose的父类transferobject
    anim.updateMetadata(metadata)# 来自Animation的父类Pose的父类transferobject，设置json 里的 metadata
    anim.save(
        path,
        time=time,
        sampleBy=sampleBy,
...
def clampRange(srcTime, dstTime):
    ...
    if srcStart > dstEnd or srcEnd < dstStart:
        msg = "The for_Maya and dst time do not overlap. " \
              "Unable to clamp (for_Maya=%s, dest=%s)"
...
def loadAnims(
    ...
    for path in paths:
        ...
        anim = utils.Animation.fromPath(path)
...
class Animation(utils.Pose): #继承自Pose，增加一些命令，上面被调用
    ...
    def select(self, objects=None, namespaces=None, **kwargs):
        ...
        :rtype: None
        """
        selectionSet = utils.SelectionSet.fromPath(self.poseJsonPath())
    ...
    def assetPath(self):
        """
        :return: list of assetPath
        """
        fn = []
        if self.metadata().get("references"):
            for i in self.metadata().get("references"):
                m = i.get("filename")
                fn.append(m)
        return fn
    ...
    def read(self, path=None):
        ...
        utils.Pose.read(self, path=path)
    ...
    def save(
        ...
        if not time:
            time = utils.selectedObjectsFrameRange(objects)
        start, end = time
        ...
        if utils.getDurationFromNodes(objects or []) <= 0:
        ...
        try:
            if bakeConnected:
                ...
                utils.bakeConnected(objects, time=(start, end), sampleBy=sampleBy)
            ...
            for name in objects:
                if maya.cmds.copyKey(name, time=(start, end), includeUpperBound=False, option="keys"):
                    ...
                    for attr in attrs:
                        dstAttr = utils.Attribute(transform, attr)
                        ...
                        if dstCurve:
                            ...
                            srcAttr = utils.Attribute(name, attr)
    ...
    def load(
        ...
        valid = False
        matches = list(utils.matchNames(srcObjects=srcObjects, dstObjects=objects, dstNamespaces=namespaces))
        ...
        if not matches or not valid:
            ...
            raise utils.NoMatchFoundError(text)
        ...
        try:
            ...
            for srcNode, dstNode in matches:
                ...
                for attr in self.attrs(srcNode.name()):
                    ...
                    dstAttr = utils.Attribute(dstNode.name(), attr)
