...
def ls(*args, **kwargs):
    ...
    :type kwargs: dict
    """
    return [utils.Node(name) for name in maya.cmds.ls(*args, **kwargs) or []]
...
def listAttr(name, **kwargs):
    ...
    :type name: str
    :type kwargs: str
    :rtype: list[utils.Attribute]
    """
    attrs = maya.cmds.listAttr(name, **kwargs) or []
    attrs = list(set(attrs))
    return [utils.Attribute(name, attr) for attr in attrs]
...
def animCurve(fullname):
    ...
    :rtype: str or None
    """
    attribute = utils.Attribute(fullname)
...
def getSelectedObjects():
    ...
    :rtype: list[str]
    :raises: utils.SelectionError:
    """
    selection = maya.cmds.ls(selection=True)
    if not selection:
        raise utils.SelectionError("No objects selected!")
    return selection
