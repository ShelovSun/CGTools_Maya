...
def savePose(path, objects, iconPath="", metadata=None):
    """
    ...
    :rtype: Pose
    """
    pose = utils.Pose.fromObjects(objects)

    if metadata:
        pose.updateMetadata(metadata)

    pose.save(path + "/pose.json")
...
class Pose(TransferObject):  # Pose 类继承自TransferObject，Animation的父类

    def __init__(self):
        utils.TransferObject.__init__(self)
    ...
    def select(self, objects=None, namespaces=None, **kwargs):
        ...
        :rtype: None
        """
        selectionSet = utils.SelectionSet.fromPath(self.path())
    ...
    def mirrorAxis(self, name):
        ...
        if result is None:
            print("Cannot find mirror axis in pose for %s", name)
    ...
    def mirrorValue(self, name, attr, mirrorAxis):
        ...
        if self.mirrorTable() and name:
            ...
            if value is not None:
                ...
                print("Cannot find mirror value for %s.%s", name, attr)
    ...
    def cacheNode(
        ...
        if usingNamespaces:
            ...
            except utils.NoObjectFoundError as msg:
                logger.debug(msg)
                return
            except utils.MoreThanOneObjectFoundError as msg:
                logger.debug(msg)
                return
        ...
        for attr in self.attrs(srcName):
            ...
            dstAttribute = utils.Attribute(dstNode.name(), attr)
            ...
            srcAttribute = utils.Attribute(dstNode.name(), attr, value=value, type=type_)
            dstAttribute.update()
