...
#
#     @staticmethod  #@classmethod 类方法和类直接进行交互，而不需要和实例进行交互，类方法传参（cls）作为方法的第一个参数
#                    #@staticmethod静态方法不需要访问任何实例化方法和属性，纯粹的通过传入参数并返回数据的功能性方法，不需要传参
#     def values():
#         return [
...
class Item(QtWidgets.QListWidgetItem):  # 被 libraryitem 继承
    ...
    def __init__(self, *args):
        QtWidgets.QListWidgetItem.__init__(self, *args)
        ...
        self._worker.setAutoDelete(False) #提交给线程池的 QRunnable 对象在它的 run() 函数执行完后会被自动 delete 掉，如果不想线程池删除它，在调用线程池的 start() 前调用 setAutoDelete(false) 即可
    ...
    def setHidden(self, value):
        ...
        :rtype: None
        """
        QtWidgets.QListWidgetItem.setHidden(self, value)
    ...
    def setIcon(self, iconpath):
        ...
        if isinstance(iconpath, six.string_types):
            if not os.path.exists(iconpath):
                icon = QtGui.QIcon("%s/icon/Default.png" % self.scriptsPath)
            else:
    ...
    def clearCache(self):
        """Clear the thumbnail cache.""" #    清理pixmap
        self._pixmap = {}
        self._pixmapRect = None
        self._pixmapScaled = None
        self._thumbnailIcon = None
    ...
    def paint(self, painter, option, index):
        ...
        try:
            ...
            if index.column() == 0 and self.sliderValue() != 0: # 绘制item 的进度条
