...
class FavoritesQListWiget(QtWidgets.QListWidget):
    ...
    def show_menu(self, point):
        ...
        if currentItem is not None:
            ...
            addTag_action = QtWidgets.QAction(u'添加标签', self)
            addTag_action.setIcon(QtGui.QIcon("{}/icon/tagPlus.png".format(self.scriptsPath)))
            addTag_action.triggered.connect(self.__scenePublish)
            addFavor_action = QtWidgets.QAction(u'添加', self)
            addFavor_action.setIcon(QtGui.QIcon("{}/icon/refresh.png".format(self.scriptsPath)))
            addFavor_action.triggered.connect(self.refresh_asset)
...
class MainQListWiget(QtWidgets.QListWidget):
    """ 主窗 """
    ...
    def __init__(self, tab="Asset"):
        super(MainQListWiget, self).__init__()
        self._tab = tab
        self._itemsDict = []
        self._isList = False
        self._itemSize = 120
    ...
    def getItemsDictFromCGTW(self, __tab, __project, __type):
        ...
        t_tw = cgtw2.tw()
        __items_dict = []
        asset, assetmaya, assetstapy, entity, cn_name = self.get_CGTW_entity(__tab)
        ...
        TW_dictionInfo = t_tw.info.get(TW_proj, asset, t_asset_ids, [entity, cn_name, "asset.image"])
        for info in TW_dictionInfo:
            ...
            bbb = {'role_name': info[entity], 'project': __project, 'type': __type,
                   'zh_name': info[cn_name], 'icon_path': info["asset.image"]}
            __items_dict.append(bbb)
...
class Item(QtWidgets.QListWidgetItem):
    ...
    def setFavor(self, value):
        self._isFavor = value
        faveJson = "%s/%s_fave.json" % (tempPath, self._tab)
        if not os.path.isfile(faveJson):
            f = open(faveJson, 'w')
    ...
    def paintIcon(self, painter, option, index, align=None):
        ...
        if self.itemsWidget().isList():
            ...
            pixmap = QtGui.QPixmap(self.itemData().get("icon_path"))
...
class PreviewLabel(QtWidgets.QLabel):
    ...
    def paintTypeIcon(self):
        ...
        typePixmap = self.typePixmap
        if typePixmap:
            painter.drawPixmap(10, 10, typePixmap)
            painter.setPen(QtCore.Qt.NoPen)
