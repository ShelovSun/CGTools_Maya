...
print(SM_scriptName, SM_scriptPath)
# scriptsPath = os.path.split(os.path.realpath(__file__))[0].replace('\\', '/')
# Parameter Initialization 
SM_MainUIFile = scriptsPath + '/springMagic.ui'
SM_paypalLink = r'https://www.paypal.me/Yanbin'
SM_linkedinLink = r'https://ca.linkedin.com/in/baiyanbin'
SM_vimeoLink = r''
SM_bilibiliLink = r'http://animbai.com/zh/2017/10/14/skintools-tutorials/'
SM_youtubeLink = r'http://animbai.com/2017/10/14/skintools-tutorials/'
SM_bitcoin = r'1HT1L4tGobHVmJJZMsj2GG1oZRaowedwUs'
SM_updateLink = r'http://animbai.com/category/download/'
...
def SM_showSpam():
    ...
    # print as unicode
    SM_printTextLable(SM_main_processLable, str(sWrod, "utf8", errors="ignore") if isinstance(sWrod, bytes) else sWrod)
...
def SM_applyLanguage(lanId):
    ...
    if lanId in lanDict.keys():
        ...
        newUIFile = SM_scriptPath + os.sep + os.path.basename(SM_MainUIFile).split('.')[0] + lanDict[lanId] + '.' + \
                    os.path.basename(SM_MainUIFile).split('.')[1]
...
springSubDiv_lineEdit = pm.textField(SM_spring_tab + '|spring_groupBox|keyRange_groupBox|springSubDiv_lineEdit',
                                     edit=True)
springLoop_checkBox = pm.checkBox(SM_spring_tab + '|spring_groupBox|keyRange_groupBox|springLoop_checkBox', edit=True)
springClearSubFrame_checkBox = pm.checkBox(
    SM_spring_tab + '|spring_groupBox|keyRange_groupBox|springClearSubFrame_checkBox', edit=True)
...
springActive_radioButton = pm.radioButton(SM_spring_tab + '|spring_groupBox|keyRange_groupBox|springActive_radioButton',
                                          edit=True)
springFrom_radioButton = pm.radioButton(SM_spring_tab + '|spring_groupBox|keyRange_groupBox|springFrom_radioButton',
                                        edit=True)
...
springCollision_checkBox = pm.checkBox(SM_spring_tab + '|spring_groupBox|collision_groupBox|springCapsule_checkBox',
                                       edit=True)
springFastMove_checkBox = pm.checkBox(SM_spring_tab + '|spring_groupBox|collision_groupBox|springFastMove_checkBox',
                                      edit=True)
springFloor_checkBox = pm.checkBox(SM_spring_tab + '|spring_groupBox|collision_groupBox|springFloor_checkBox',
                                   edit=True)
springFloor_lineEdit = pm.textField(SM_spring_tab + '|spring_groupBox|collision_groupBox|springFloor_lineEdit',
                                    edit=True)

springBindPose_button = pm.button(SM_spring_tab + '|spring_groupBox|springBonePose_groupBox|springBindPose_button',
                                  edit=True)
springStraight_button = pm.button(SM_spring_tab + '|spring_groupBox|springBonePose_groupBox|springStraight_button',
                                  edit=True)
...
def SM_checkUpdate():
    ...
    try:
        page_source = urlopen(SM_versionCheckLink, timeout=5).read()
    except:
        pass
...
def SM_goShelf():
    ...
    pm.shelfButton(commandRepeatable=True, image1=SM_scriptPath + os.sep + "icons" + os.sep + "Title.png",
                   label="Spring Magic", parent=parentTab,
...
def springBindController():
    ...
    for joint in proxyJointLst:
        ...
        if idx + 1 < len(ctrlList):
            cns = pm.aimConstraint(ctrlList[idx + 1], proxyJointLst[idx], aimVector=[1, 0, 0], upVector=[0, 0, 0],
...
def SpringMagicMaya(
    ...
    while i < ((endFrame - startFrame) + (1 / subDiv)):
        ...
        for obj in objs:
            ...
            if len(obj.listRelatives(children=True)) > 0 and (
                    obj.getParent() is not None):  # skip end bone and root bone
                    ...
                    if i > 0 or isSeconendPath:  # only skip first frame of first calculation path
                        ...
                        if isCollision and capsuleLst:
                            if preCheckCollision(cur_obj_pos, boneLength, capsuleLst):
                                ...
                                col_pre, col_body_pre, hitCylinder_pre = checkCollision(cur_child_pos, prev_child_pos,
                                                                                        capsuleLst, True)
                                # check col from cur pos to previous pos

                                col_cur, col_body_cur, hitCylinder_cur = checkCollision(cur_child_pos, prev_child_pos,
                                                                                        capsuleLst, False)
                        ...
                        if tension > 0 and prev_grandChild_pos and hasChildCollide:
                            aim_by_ratio(obj, prev_child_pos, cur_child_pos, ratio, upVector, prev_grandChild_pos,
                                         tension)
                        else:
                        ...
                        obj_trans_dict[obj] = [cur_obj_pos, cur_child_pos, r, cur_grandChild_pos, hasChildCollide,
                                               boneLength, upVector]
                        ...
                        obj_trans_dict[obj] = [prev_obj_pos, prev_child_pos, prev_rot, prev_grandChild_pos, False,
                                               boneLength, prev_obj_yAxis]
