...
def bindControls(linked_chains=False):
    ...
    if linked_chains:
        ...
        ctrls_chains_list = map(
            lambda x: [x] + [y for y in pm.listRelatives(x, allDescendents=True) if y in selected_ctrls][::-1],
            top_hierarchy_ctrls_list)
...
def SpringMagicMaya(objs, spring, springMagic, progression_callback=None):
    ...
    transforms_chains_list = map(
        lambda x: ([x] if x.getParent() else []) + [y for y in pm.listRelatives(x, allDescendents=True) if y in objs][
                                                   ::-1], top_hierarchy_joints_list)
    ...
    for frame in frame_generator:
        ...
        progression = progression_generator.__next__()
        progression = clamp(progression, 0, 100)
