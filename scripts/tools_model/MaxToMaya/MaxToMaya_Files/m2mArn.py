...
class xmlToArnold():
	...
	def set_renderer(self):
		...
		pm.setAttr("defaultRenderGlobals.currentRenderer", "mtoa") #Set Renderer
		...
		for tex in textures:
			...
			if texture_node:
				...
				try:
					...
					if tex.attrib["shaderName"] == 'diffuse_map': connectNode('.outColor', '.baseColor')
					if tex.attrib["shaderName"] == 'vraylightlightmap': connectNode('.outColor', '.color')
					if tex.attrib["shaderName"] == 'opacity_map': connectNode('.outColor', '.opacity')
					if tex.attrib["shaderName"] == 'vraylightlightmap': connectNode('.outColor', '.emissionColor')
					if tex.attrib["shaderName"] == 'reflection_map': connectNode('.outColor', '.specularColor')
					if tex.attrib["shaderName"] == 'vrayreflection_glossiness_map':
						connectNode('.outAlpha', '.specularRoughness')
						try:
							cmds.setAttr(texture_node + '.invert', 1)
							cmds.setAttr(texture_node + '.alphaIsLuminance', 1)
						except:
							pass
					if 'bump_map' in tex.attrib["shaderName"]:
						try:
							...
							cmds.setAttr(bump_node + '.bumpDepth', bumpValue)
	...
	def settingsFromXMLtoMat(self, matNew, matOld):
		...
		for material in self.root.iter('vraylightmtl'):
			...
			if matOld == matName and matType == 'VRayLightMtl':
				cmds.setAttr(matNew + '.emissionColor', *self.rgbConv(xmlMat.get('color')), type="float3")
				cmds.setAttr(matNew + '.emission', float(xmlMat.get('multiplier')) / 10 )
		...
		for material in self.root.iter('material'):
			try:
				...
				if matOld == matName and matType == 'VRayMtl':
					cmds.setAttr(matNew + '.baseColor', *self.rgbConv(xmlMat.get('diffuse_color')), type="float3")
					...
					cmds.setAttr(matNew + '.transmissionExtraRoughness', 1 - float(xmlMat.get('vrayrefraction_glossiness')))
					cmds.setAttr(matNew + '.specularIOR', float(xmlMat.get('vrayrefraction_ior')))

					cmds.setAttr(matNew + '.specularColor', *self.rgbConv(xmlMat.get('vrayreflection_color')), type="float3")
					cmds.setAttr(matNew + '.specularRoughness', 1 - float(xmlMat.get('vrayreflection_glossiness')))
					cmds.setAttr(matNew + '.transmissionColor', *self.rgbConv(xmlMat.get('vrayrefraction_color')), type="float3")
					if xmlMat.get('vrayreflection_fresnel') == 'false': cmds.setAttr(matNew + '.specularIOR', 0)
		
				#IF MATERIAL IN XML: STANDARD MATERIAL
				if matOld == matName and matType == 'Standardmaterial':
					cmds.setAttr(matNew + '.baseColor', *self.rgbConv(xmlMat.get('diffuse_color')), type="float3")
					cmds.setAttr(matNew + '.specularRoughness', 0.5)
			except Exception as e:
	...
	def replaceWithMat(self, matOld, shadingGroup, matType):
		...
		if matType == 'Standardmaterial':
			cmds.shadingNode('aiStandardSurface', name = newMat, asShader=True)
		if matType == 'VRayMtl':
			cmds.shadingNode('aiStandardSurface', name = newMat, asShader=True)
		...
		try:
			...
			if light.attrib["v_type"] != 'sun':
				if light.attrib["v_area_type"] == 'Plane' and light.attrib["v_type"] == 'area':
					light_node = pm.createNode('aiAreaLight', p=light.attrib["name"])
					pm.sets('defaultLightSet', forceElement=light.attrib["name"])
					cmds.setAttr(lightName + '.exposure' , 15)
					...
					cmds.setAttr(lightName + '.color' , 255,0,0 , type="float3")
	...
	def create_light_nodeIES(self, light): #=============== IES LIGHT ======================================================
		try:
			lightName = light.attrib["name"]
			light_node = pm.createNode('aiPhotometricLight', p=light.attrib["name"])
			pm.sets('defaultLightSet', forceElement=light.attrib["name"])
			cmds.setAttr(lightName +'.aiFilename' , light.attrib["ies_file"], type="string")
			cmds.setAttr(lightName +'.intensity' , float(light.attrib["power"]) / 1000)
	...
	def convertMats(self):
		...
		for sg in shadingGroup_list:
			...
			if surfaceShader:
				print("MATERIAL:   " + surfaceShader[0])
