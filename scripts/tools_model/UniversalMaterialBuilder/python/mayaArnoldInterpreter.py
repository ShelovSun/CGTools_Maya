def createAiMaterial():
    ...
    # create Material
    aiMaterial = cmds.shadingNode("aiStandardSurface", asShader=True, n=name)
    aiMaterialSG = cmds.sets(renderable=True, noSurfaceShader=True, empty=True, n=name + "_SG")
    ...
    if "diffuse_dict" in information_dict:
        ...
        if fullControl:
            diffuse_cc = cmds.shadingNode("aiColorCorrect", asTexture=True, n="diffuse_cc")
            cmds.connectAttr(diffuseFile + ".outColor", diffuse_cc + ".input")
            cmds.connectAttr(diffuse_cc + ".outColor", aiMaterial + ".baseColor")

        else:
            cmds.connectAttr(diffuseFile + ".outColor", aiMaterial + ".baseColor")
    ...
    if "ao_dict" in information_dict:
        ...
        if fullControl:
            ...
            cmds.connectAttr(ao_remap + ".outValue", aiMaterial + ".base")

        else:
            cmds.connectAttr(aoFile + ".outAlpha", aiMaterial + ".base")
    ...
    if "reflection_dict" in information_dict:
        ...
        if fullControl:
            ...
            cmds.connectAttr(reflection_remap + ".outValue", aiMaterial + ".specular")

        else:
            cmds.connectAttr(reflectionFile + ".outAlpha", aiMaterial + ".specular")
    ...
    if "roughness_dict" in information_dict:
        ...
        if fullControl:
            ...
            cmds.connectAttr(roughness_remap + ".outValue", aiMaterial + ".specularRoughness")

        else:
            cmds.connectAttr(roughnessFile + ".outAlpha", aiMaterial + ".specularRoughness")
    ...
    if "metalness_dict" in information_dict:
        ...
        if fullControl:
            ...
            cmds.connectAttr(metalness_remap + ".outValue", aiMaterial + ".metalness")

        else:
            cmds.connectAttr(metalnessFile + ".outAlpha", aiMaterial + ".metalness")
    ...
    if "refraction_dict" in information_dict:
        ...
        if fullControl:
            ...
            cmds.connectAttr(refraction_remap + ".outValue", aiMaterial + ".transmission")

        else:
            cmds.connectAttr(refractionFile + ".outAlpha", aiMaterial + ".transmission")
            
    # create subsurface
    if "subsurface_dict" in information_dict:
        cmds.setAttr(aiMaterial + ".subsurface", 1)
        ...
        if fullControl:
            subsurface_cc = cmds.shadingNode("aiColorCorrect", asTexture=True, n="subsurface_cc")
            cmds.connectAttr(subsurfaceFile + ".outColor", subsurface_cc + ".input")
            cmds.connectAttr(subsurface_cc + ".outColor", aiMaterial + ".subsurfaceColor")

        else:
            cmds.connectAttr(subsurfaceFile + ".outColor", aiMaterial + ".subsurfaceColor")

    # create bump
    if "bump_dict" in information_dict:
        bumpFile = cmds.shadingNode("file", asTexture=True, n=bumpName)
        bumpNode = cmds.shadingNode("aiBump2d", asTexture=True, n=bumpName + "_bumpNode")
        ...
        if fullControl:
            ...
            cmds.connectAttr(bump_remap + ".outValue", bumpNode + ".bumpMap")

        else:
            cmds.connectAttr(bumpFile + ".outAlpha", bumpNode + ".bumpMap")
    ...
    if "normal_dict" in information_dict:
        ...
        normalNode = cmds.shadingNode("aiNormalMap", asTexture=True, n=normalName + "_normalNode")
    ...
    if "emission_dict" in information_dict:
        ...
        if fullControl:
            emission_cc = cmds.shadingNode("aiColorCorrect", asTexture=True, n="emission_cc")
            cmds.connectAttr(emissionFile + ".outColor", emission_cc + ".input")
            cmds.connectAttr(emission_cc + ".outColor", aiMaterial + ".emissionColor")

        else:
            cmds.connectAttr(emissionFile + ".outColor", aiMaterial + ".emissionColor")
    ...
    if "opacity_dict" in information_dict:
        ...
        if fullControl:
            opacity_cc = cmds.shadingNode("aiColorCorrect", asTexture=True, n="opacity_cc")
            cmds.connectAttr(opacityFile + ".outColor", opacity_cc + ".input")
            cmds.connectAttr(opacity_cc + ".outColor", aiMaterial + ".opacity")

        else:
            cmds.connectAttr(opacityFile + ".outColor", aiMaterial + ".opacity")
    ...
    if assignMaterial == 1:
        if len(sel) > 0:
            cmds.sets(sel, edit=True, forceElement=aiMaterialSG)
