...
scriptsPath = os.path.split(os.path.realpath(__file__))[0].replace('\\', '/').replace('/tools_model', '')


def showMenu():
    menu = QtWidgets.QMenu()
    action_a = QtWidgets.QAction(QtGui.QIcon("{}/icons/publish.png".format(scriptsPath)), u"Mod Publish", menu)
    # action_a.setIcon(QtGui.QIcon("{}/icons/publish.png".format(scriptsPath)))
    action_a.triggered.connect(command_a)

    action_d = QtWidgets.QAction(u"Scene Publish", menu)
    action_d.setIcon(QtGui.QIcon("{}/icons/publish.png".format(scriptsPath)))
    action_d.triggered.connect(command_d)

    action_f = QtWidgets.QAction(u"资产重命名工具", menu)
    action_f.triggered.connect(command_f)

    action_b = QtWidgets.QAction(u"Material Manager", menu)
    action_b.triggered.connect(command_b)

    action_c = QtWidgets.QAction(u"braid Maker", menu)
    action_c.triggered.connect(command_c)

    action_e = QtWidgets.QAction(u"Snow Factory", menu)
...
def command_e():
    import tools_mod.SnowFactory.SnowFactory as SF
    SF.snowFactoryWindow()


def command_f():
    import tools_mod.renameMat.renameMat as rm
    rm.showWindow()
