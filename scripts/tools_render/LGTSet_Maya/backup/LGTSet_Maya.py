...
class LGTSetToolsUI():
    ...
    def init_ui(self):
        ...
        cmds.window(Wins, title='LGT Set Tool v1.0.1', w=300)
        form = cmds.formLayout()
        tabs = cmds.tabLayout(innerMarginWidth=5, innerMarginHeight=5)
        cmds.formLayout(form, edit=True,
                        attachForm=((tabs, 'top', 0), (tabs, 'left', 0), (tabs, 'bottom', 0), (tabs, 'right', 0)))
        child1 = cmds.columnLayout(columnAttach=('both', 5), rowSpacing=1, columnWidth=300)
        cmds.scrollLayout(w=530, h=450)
        cmds.scriptJob(parent=Wins, event=['renderLayerManagerChange', 'GetAovs_Rs()'])
        cmds.columnLayout(adjustableColumn=True, rowSpacing=2)
        cmds.picture(image=icoPath + 'LayerTool.png')
        ...
        cmds.textFieldGrp('CHRsNameUI', label='Name', text='GRP', cw2=[60, 120])
        cmds.setParent('..')
        ...
        cmds.textFieldGrp('OldNameUI', label='Old', text='GRP', cw2=[25, 100])
        cmds.textFieldGrp('NewNameUI', label='New', text='GRP', cw2=[25, 100])
        cmds.setParent('..')
        cmds.button(label='Rename', bgc=[0.36, 0.36, 0.36], h=30, w=80, c='renameRenderPSD_UI()')
        cmds.separator(height=20, style='in', horizontal=True)
        ...
        cmds.button(label='Global Setting', bgc=[0.1, 0.5, 0.1], h=30, w=200, c='Global_Setting_Arnold()')
        cmds.separator(height=20, style='in', horizontal=True)
        cmds.rowColumnLayout(numberOfColumns=5)
        ...
        cmds.menuItem(label='AN')
        cmds.menuItem(label='BG')
        cmds.textFieldGrp('CHAiNameUI', label='Name', text='GRP', cw2=[60, 120])
        cmds.setParent('..')
        cmds.scrollLayout('scrollLayout', w=100, h=30)
        cmds.rowColumnLayout(numberOfColumns=5)
        cmds.checkBox('COLUI', label='COL', v=True)
        ...
        cmds.setParent('..')
        cmds.setParent('..')
        cmds.button('Create Layers', bgc=[0.36, 0.36, 0.36], h=30, w=200, c='LayerCreator_Arnold()')
        cmds.separator(height=20, style='in', horizontal=True)
        cmds.rowColumnLayout(numberOfColumns=2)
        cmds.textFieldGrp('OldNameUI', label='Old', text='GRP', cw2=[25, 100])
        cmds.textFieldGrp('NewNameUI', label='New', text='GRP', cw2=[25, 100])
        ...
        cmds.tabLayout(tabs, edit=True, tabLabel=((child1, 'RedShift'), (child2, 'Arnold'), (child3, 'Lighting')))
        cmds.setParent('..')
        cmds.setParent('..')
        cmds.setParent('..')
        cmds.showWindow(Wins)

    def ImportLighting(file):
        lGT = {'Rs_CH_LGT.ma': 'Rs_CH_LGT', 'SKBT_CH_LGT.ma': 'LgtRg_Mstr_Ctrl'}
        LGTFile = importFilesPath + file
...
class Redshift():
    ...
    def LayerAovSet_Rs(aovname, allLight=0):
