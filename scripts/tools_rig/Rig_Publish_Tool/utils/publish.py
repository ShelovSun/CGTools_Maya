...
class Publish(object):
    ...
    def removeUnloadReference(self):
        u"""
        移除没加载的 reference
    ...
    def removeAllDisplayLayer(self):
        u"""
        移除所有显示层
    ...
    def removeModelChangeError(self):
        """
        remove error
    ...
    def snapshot(self, path, imageName, need_createHistory = True):
        u"""
        截图并重命名

        """
        print("=============",need_createHistory)
        if need_createHistory == True:
            self.createHistory(path)
        else:
            pass
        cmds.playblast(frame=1, percent=100, quality=100, framePadding=1, widthHeight=[200, 200], format='image', compression='png', showOrnaments=False, viewer=False, filename='%s/%s' % (path, imageName))
        newpath = '%s/%s.png' % (path, imageName)
        if os.path.isfile(newpath):
            os.remove(newpath)
            os.rename('%s/%s.0.png' % (path, imageName), '%s/%s.png' % (path, imageName))
        else:
            os.rename('%s/%s.0.png' % (path, imageName), '%s/%s.png' % (path, imageName))
        return '%s/%s.png' % (path, imageName)
    ...
    def playblastGif(self, path, imageName):
        ...
        os.system(("D:/Project/Scripts/Tools/ffmpeg-N-102494-g2899fb61d2-win64-gpl-shared/bin/ffmpeg -i {0}ActionGif.avi -s 160x160 -vf {1} {0}ActionGif.gif").format(MYPREFSDIR,cdec)) 
        outputAvipath=path
        outputGifpath=path
        #os.system("D:/Project/Scripts/Tools/ffmpeg-N-102494-g2899fb61d2-win64-gpl-shared/bin/ffmpeg -i G:/xxx.avi -b:v 640k G:/xxx.gif") 
    
    def createModAST(self):
        u"""
        创建 AST 组
        :return:
        """
    ...
    def repathNormalMap(self, path):
        ...
        for i in set(normalMapFileList):
            ...
            if normalMapFileName.find('<UDIM>') != -1:
                udimMapList = cmds.getFileList(folder=normalMapFileDir, filespec=normalMapFileName.replace('<UDIM>', '*'))
                if udimMapList != []:
